...
export default (router: express.Router) => {
    router.post('/api/users/lib/save/:sessionId', express.json({limit: '1mb'}), isAuthenticated, isOwner, saveTestToLib);
    ...
    router.post('/api/users/lib/create-test-pdf/:sessionId', express.json({limit: '1mb'}), isAuthenticated, isOwner, generateTestPDF);
    router.get('/api/users/lib/export-test-pdf/:sessionId/:testId', isAuthenticated, isOwner, exportTestPDF);
    router.post('/api/users/lib/create-answers-pdf/:sessionId', express.json({limit: '1mb'}), isAuthenticated, isOwner, generateTestAnswersPDF);
